...
public class TutorialText : MonoBehaviour
{
    ...
    public void updateText(float _stageNum, float _stageKey)
    {
        switch (_stageNum)
        {
            ...
            case (1f): // Left
                {
                    gameObject.GetComponent<UnityEngine.UI.Text>().text = "Hold W to move forward."; // instructions given to player
            ...
            case (1.2f): // Right
                {
                    gameObject.GetComponent<UnityEngine.UI.Text>().text = "Hold A to turn left.";
                    if (_stageKey == 1.2f)
                    {
                        stageNum = 1.3f;
                        updateText(stageNum, 0);
            ...
            case (1.4f): // Jump
                {
                    gameObject.GetComponent<UnityEngine.UI.Text>().text = "Hold D to turn right.";
                    if (_stageKey == 1.4f)
                    {
                        stageNum = 1.5f;
                        updateText(stageNum, 0);
            ...
            case (1.5f):
                {
                    gameObject.GetComponent<UnityEngine.UI.Text>().text = "Good! You know how to walk!";
                    nextStageNum = -1f;
                    Invoke("progressStage", 2.0f);
                    return;
                }
